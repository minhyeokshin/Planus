...
@Log4j2
@SpringBootTest
public class GitIssueServiceTests {

    @Autowired
    private GitHubIssueService gitHubIssueService;

    @Test
    @DisplayName("이슈 목록 스케줄링 테스트")
    @Transactional
    public void testIssueListSchedule() {
        // when
        gitHubIssueService.issueListSchedule();
    ...
    public void testGetIssueList() {
        ...
        log.info("issueList.size() = " + issueList.size());
        
        // 이슈 목록이 null이 아닌지 확인
        assertNotNull(issueList);
        
        // 이슈 목록이 비어있지 않다면, 각 이슈의 필수 필드가 존재하는지 확인
        if (!issueList.isEmpty()) {
            IssueDTO firstIssue = issueList.get(0);
            assertNotNull(firstIssue.getIssueId());
            assertNotNull(firstIssue.getIssueTitle());
            assertNotNull(firstIssue.getUserName());
            assertNotNull(firstIssue.getIssueDate());
            assertNotNull(firstIssue.getIssueStatus());
            assertNotNull(firstIssue.getIssueURL());
        }
    }
}
